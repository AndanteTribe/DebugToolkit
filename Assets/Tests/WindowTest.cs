...
namespace DebugToolkit.Tests
{
    public class WindowTest
    {
        ...
        public async Task MasterWindow_CanBeMinimized()
        {
            ...
            Assert.That(windowContent.style.display.value, Is.EqualTo(DisplayStyle.Flex), "Window content should be visible initially.");

            var mouse = InputSystem.AddDevice<Mouse>();
            _input.Set(mouse.position, new Vector2(422, 995));
            _input.Click(mouse.leftButton);
            await Awaitable.NextFrameAsync();
            await Awaitable.NextFrameAsync();

            Assert.That(windowContent.style.display.value, Is.EqualTo(DisplayStyle.None), "Window content should be hidden after minimizing.");
            Assert.That(minimizeButton.text, Is.EqualTo("^"), "Minimize button text should change to '^' when minimized.");

            _input.Click(mouse.leftButton);
            await Awaitable.NextFrameAsync();
            await Awaitable.NextFrameAsync();

            Assert.That(windowContent.style.display.value, Is.EqualTo(DisplayStyle.Flex), "Window content should be visible after maximizing.");
            Assert.That(minimizeButton.text, Is.EqualTo("-"), "Minimize button text should change to '-' when maximized.");
        }
        ...
        {
            var masterWindows = _debugViewWindowTest.Root.Query<VisualElement>(className: DebugConst.ClassName + "__master-window").ToList();
        ...
        {
            var toggleAllButton = _debugViewWindowTest.Root.Q<Button>(className: DebugConst.ClassName + "__toggle-all-button");
        ...
        public async Task ToggleAllButton_TogglesAllWindowsVisibility()
        {
            ...
            var mouse = InputSystem.AddDevice<Mouse>();
            _input.Set(mouse.position, new Vector2(15,20));
            _input.Click(mouse.leftButton);
            await Awaitable.NextFrameAsync();
            await Awaitable.NextFrameAsync();
            ...
            foreach (var window in DebugViewerBase.DebugWindowList)
            {
                Assert.That(window.style.display.value, Is.EqualTo(DisplayStyle.None), "Window should be hidden after toggle all.");
            }
            ...
            foreach (var window in DebugViewerBase.DebugWindowList)
            {
                Assert.That(window.style.display.value, Is.Not.EqualTo(DisplayStyle.None), "Window should be visible after toggling back.");
            }
        }
        ...
        {
            var toggleAllButtons = _debugViewWindowTest.Root.Query<Button>(className: DebugConst.ClassName + "__toggle-all-button").ToList();
        ...
        public async Task Window_BringsToFrontWhenClicked()
        {
            ...
            var mouse = InputSystem.AddDevice<Mouse>();
            _input.Set(mouse.position, new Vector2(476, 894));
            _input.Click(mouse.leftButton);
            await Awaitable.NextFrameAsync();
            await Awaitable.NextFrameAsync();
            ...
            _input.Set(mouse.position, new Vector2(557, 828));
            _input.Click(mouse.leftButton);
            await Awaitable.NextFrameAsync();
            await Awaitable.NextFrameAsync();
        ...
        [TestCase("TestWindow1",110, 930)]
        [TestCase("TestWindow2",112, 872)]
        public async Task WindowCloseButton_HidesWindowAndUpdatesToggle(string windowName, float screenPosX, float screenPosY)
        {
            var toggle = DebugViewerBase.MasterWindow.Q<ScrollView>(className: DebugConst.WindowListClassName)
                .Query<Toggle>(className: DebugConst.ToggleWindowDisplayClassName)
                .Where(t => t.text == windowName).First();
            ...
            Assert.That(window.style.display.value, Is.EqualTo(DisplayStyle.Flex), "Window should be visible initially.");
            Assert.That(toggle.style.backgroundColor.value, Is.EqualTo(new Color(0.4f, 0.8f, 0.4f)), "Toggle color should indicate visible window.");
            ...
            Assert.That(window.style.display.value, Is.EqualTo(DisplayStyle.None), "Window should be hidden after clicking close button.");
            Assert.That(toggle.style.backgroundColor.value, Is.EqualTo(new Color(0.6f, 0.2f, 0.2f)), "Toggle color should indicate hidden window.");
        }
